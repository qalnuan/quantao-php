...
class StoreDine extends BaseModel
{
   ...
   /**
    ...
    public static function getbadge($where,$type){
        ...
        return [
            ...
            [
                'name'=>'霸王餐成功商品件数',
                'field'=>'件',
                'count'=>self::getModelTime($where,$StoreOrderModel)->where('dine_id','<>',0)->sum('total_num'),
                'content'=>'霸王餐成功商品总件数',
                'background_color'=>'layui-bg-green',
                'sum'=>$StoreOrderModel->where('dine_id','<>',0)->sum('total_num'),
    ...
    public static function getMaxList($where){
        ...
        $model=StoreOrder::alias('a')->join('store_dine b','b.id=a.dine_id')->where('a.paid',1);
        $list=self::getModelTime($where,$model,'a.add_time')->group('a.dine_id')->order('p_count desc')->limit(10)
            ->field(['count(a.dine_id) as p_count','b.title as store_name','sum(b.price) as sum_price'])->select();
    ...
    public static function ProfityTop10($where){
        $classs=['layui-bg-red','layui-bg-orange','layui-bg-green','layui-bg-blue','layui-bg-cyan'];
        $model = StoreOrder::alias('a')->join('store_dine b','b.id = a.dine_id')->where('a.paid',1);
        $list=self::getModelTime($where,$model,'a.add_time')->group('a.dine_id')->order('profity desc')->limit(10)
            ->field(['count(a.dine_id) as p_count','b.title as store_name','sum(b.price) as sum_price','(b.price-b.cost) as profity'])
            ->select();
        if(count($list)) $list=$list->toArray();
        $maxList=[];
        $sum_count=0;
    ...
    public static function getBargainRefundList($where = array()){
        $model = StoreOrder::alias('a')->join('store_dine b','b.id=a.dine_id');
        $list = self::getModelTime($where,$model,'a.add_time')->where('a.refund_status','<>',0)->group('a.dine_id')->order('count desc')->page((int)$where['page'],(int)$where['limit'])
            ->field(['count(a.dine_id) as count','b.title as store_name','sum(b.price) as sum_price'])->select();
        if(count($list)) $list=$list->toArray();
        return $list;
    }
...
//        $model = $model->join('StoreProduct p','p.id=s.product_id');
    ...
    public static function SaveExcel($where){
        ...
        PHPExcelService::setExcelHeader(['编号','活动标题','活动简介','原价','霸王餐价','库存','霸王餐状态','结束时间','状态'])
            ->setExcelTile('霸王餐产品导出',' ',' 生成时间：'.date('Y-m-d H:i:s',time()))
     ...
     * @return array
     */
    public static function getDineIdAll($where = array()){
        $model = new self;
     ...
     * @return int|string
     */
    public static function getDineCount($where = array()){
        $model = new self;
        $model = self::isWhere($where,$model);
        $model = $model->where('is_del',0);
        return $model->count();
     ...
     * @return mixed
     */
    public static function getDineField($id,$field = 'title'){

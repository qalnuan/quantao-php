...
class StoreDineAttr extends BaseModel
{
    /**
     * 模型名称
     * @var string
     */
    protected $name = 'store_dine_attr';
    ...
    public static function createProductAttr($attrList,$valueList,$productId)
    {
        ...
        $attrModel = new self;
        $attrValueModel = new StoreDineAttrValue;
        self::beginTrans();
        ...
        && false !== StoreDineAttrResult::setResult($result,$productId);
        self::checkTrans($res);
        if($res)
            return true;
        else
    ...
    public static function clearProductAttr($productId)
    {
        ...
        $res = false !== self::where('product_id',$productId)->delete()
            && false !== StoreDineAttrValue::clearProductAttrValue($productId);
        if(!$res)

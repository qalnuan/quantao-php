...
class Index extends AuthController
{
    ...
    public function main()
    {
        ...
        $topData['stockProduct'] = StoreProduct::where('stock','<=',$replenishment_num)->where('is_show',1)->where('is_del',0)
            ->where('mer_id', $this->adminId)->count();
        //待处理提现
        $topData['treatedExtract'] = UserExtractModel::where('status',0)->count();


        //订单数->昨日
        $now_day_order_p = StoreOrderModel::where('paid',1)
            ->where('mer_id', $this->adminId)->whereTime('pay_time','yesterday')->count();
        $pre_day_order_p = StoreOrderModel::where('paid',1)
            ->where('mer_id', $this->adminId)->where('pay_time','>',$pre_day)->where('pay_time','<',$now_day)->count();
        $first_line['d_num'] = [
        ...
        //交易额->昨天
        $now_month_order_p = StoreOrderModel::where('paid',1)
            ->where('mer_id', $this->adminId)->whereTime('pay_time','yesterday')->sum('pay_price');
        $pre_month_order_p = StoreOrderModel::where('paid',1)
            ->where('mer_id', $this->adminId)->where('pay_time','>',$beforyester_day)->where('pay_time','<',$pre_day)->sum('pay_price');
        $first_line['d_price'] = [
        ...
        //交易额->月
        $now_month_order_p = StoreOrderModel::where('paid',1)
            ->where('mer_id', $this->adminId)->whereTime('pay_time','month')->sum('pay_price');
        $pre_month_order_p = StoreOrderModel::where('paid',1)
            ->where('mer_id', $this->adminId)->where('pay_time','>',$pre_month)->where('pay_time','<',$now_month)->value('sum(pay_price)');
        $first_line['m_price'] = [
            'data' => $now_month_order_p > 0 ? $now_month_order_p : 0,
            'percent' => abs($now_month_order_p - $pre_month_order_p),
            'is_plus' => $now_month_order_p - $pre_month_order_p > 0 ? 1 : ($now_month_order_p - $pre_month_order_p == 0 ? -1 : 0)
        ...
        //本月订单总数
        $now_order_info_c = StoreOrderModel::where('add_time','>',$now_month)
            ->where('mer_id', $this->adminId)->count();
        $pre_order_info_c = StoreOrderModel::where('add_time','>',$pre_month)
            ->where('mer_id', $this->adminId)->where('add_time','<',$now_month)->count();
        $order_info['first'] = [
        ...
        $now_order_info_c = StoreOrderModel::where('add_time','>',$pre_month)->where('add_time','<',$now_month)
            ->where('mer_id', $this->adminId)->count();
        $pre_order_info_c = StoreOrderModel::where('add_time','>',$second_pre_month)->where('add_time','<',$second_now_month)
            ->where('mer_id', $this->adminId)->count();
        $order_info["second"] = [
            'data' => $now_order_info_c ? $now_order_info_c : 0,
            'percent' => abs($now_order_info_c - $pre_order_info_c),

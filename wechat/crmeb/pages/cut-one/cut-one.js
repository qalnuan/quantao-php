    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.code == 200) {
          ...
          if (that.data.product.description){
            WxParse.wxParse('description', 'html', that.data.product.description, that, 0);
          }
          if (that.data.product.rule){
            WxParse.wxParse('rule', 'html', that.data.product.rule, that, 0);
          }
        } else {
          that.setData({
            product: []
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.code == 200) {
          ...
          if (that.data.bargainUid != app.globalData.uid) {
            wx.showToast({
              title: '您不能再帮' + that.data.bargainUserInfo.nickname + '砍价了',
              icon: 'none',
              duration: 2000
            })
            that.setData({

...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        that.setData({
          ...
          pinkbanner: res.data.data.pinkbanner,
          secbanner: res.data.data.secbanner,
          newbanner: res.data.data.newbanner,
          recommendLsit: res.data.data.best,
          newList: res.data.data.new,
          lovely: res.data.data.lovely,
          likeList: res.data.data.hot,
          pinkList: res.data.data.pink,
  ...
  formSubmit: function (val) {
    wx.switchTab({
      url: "/pages/productSort/productSort?value=" + val.detail.value,
    })
  },
  ...
  getTime: function () {
    let that = this;
    let killList = that.data.seckillList;
    let len = killList.length || 0;//时间数据长度
    function nowTime() {//时间函数
      for (var i = 0; i < len; i++) {
        var intDiff = killList[i].stop_time - Date.parse(new Date()) / 1000;//获取数据中的时间戳的时间差；
        ...
        killList[i].difftime = str;//在数据中添加difftime参数名，把时间放进去
      }
      that.setData({
        seckillList: killList
      })
    }
    nowTime();
   ...
   */
  onShow: function () {
    // console.log(this.data.killList)//数据

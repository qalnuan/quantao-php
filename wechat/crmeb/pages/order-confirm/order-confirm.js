...
Page({
      ...
      success: function (res) {
        if (res.data.code == 200) {
          if (res.data.data.length < 1) {
            ...
            wx.navigateTo({ //跳转至指定页面并关闭其他打开的所有页面（这个最好用在返回至首页的的时候）
              url: '/pages/address/address?cartId=' + that.data.cartId
            })
          }
        }
      }
  ...
  getCoupon:function(){
    var that = this;
    wx.navigateTo({ //跳转至指定页面并关闭其他打开的所有页面（这个最好用在返回至首页的的时候）
      url: '/pages/coupon-status/coupon-status?cartId=' + that.data.cartId + '&totalPrice=' + that.data.priceGroup.totalPrice
    })
    console.log(this.data.usableCoupon);
  },
  toBuy:function(){
    wx.switchTab({
  ...
  toAddress:function(){
    wx.navigateTo({ //跳转至指定页面并关闭其他打开的所有页面（这个最好用在返回至首页的的时候）
      url: '/pages/addaddress/addaddress?cartId=' + this.data.cartId
    })
  },

      ...
      <Tab v-for="(item, index) in timeList" :key="index">
        ...
        <div class="list">
          <div
            ...
            v-for="(itemDine, indexDine) in dineList"
            :key="indexDine"
          >
            <div class="pictrue">
              <img :src="itemDine.image" />
            </div>
            <div class="text acea-row row-column-around">
              <div class="line1" v-text="itemDine.title"></div>
              <div class="money">
                限时价<span
                  class="num font-color-red"
                  v-text="'￥' + itemDine.price"
                ></span>
              </div>
              <div class="progress cart-color">
                <div
                  class="bg-red"
                  :style="{ width: loading ? itemDine.percent + '%' : '' }"
                ></div>
                <div
                  class="piece font-color-red"
                  v-text="'本次' + itemDine.num + '件'"
                ></div>
              </div>
            </div>
            <div
              class="grab bg-color-red"
              v-if="item.status === 1 && itemDine.stock > 0"
              @click="goDetail(itemDine.id)"
            >
              马上抢
            </div>
            <div class="grab" v-if="item.status === 1 && itemDine.stock <= 0">
              已售磬
            </div>
...
<script>
import { getDineConfig, getDineList } from "@api/activity";
...
export default {
  ...
  methods: {
    mountedStart: function() {
      var that = this;
      getDineConfig().then(res => {
        that.$set(that, "headerImg", res.data.lovely);
        that.$set(that, "timeList", res.data.dineTime);
        that.$set(that, "active", res.data.dineTimeIndex);
      ...
      this.$scroll(this.$refs.container, () => {
        !this.loadingList && this.getDineList();
      });
      setTimeout(function() {
        that.loading = true;
      }, 500);
    ...
    getDineList: function() {
      ...
      getDineList(time, { page: that.page, limit: that.limit }).then(res => {
        that.status = res.data.length < that.limit;
        that.dineList.push.apply(that.dineList, res.data);
        that.page++;

...
class User extends ModelBasic
{
    ...
    public static function backOrderBrokerage($orderInfo)
    {
        ...
        if(!User::be(['uid'=>$userInfo['spread_uid'],'is_promoter'=>1]))
            return true;
        ...
        $mark = '一级推广人'.$userInfo['nickname'].'成功消费'.floatval($orderInfo['pay_price']).'元,奖励推广佣金'.floatval($brokeragePrice);
        self::beginTrans();
    ...
    public static function backOrderBrokerageTwo($orderInfo){
        ...
        if(!User::be(['uid'=>$userInfoTwo['spread_uid'],'is_promoter'=>1]))
            return true;

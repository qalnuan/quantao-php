...
//        $model = $model->where('c.stop_time','GT',time()-86400);
    ...
    public static function getCombinationBest($limit=0)
    {
        return self::alias('A')->join('store_product B','A.product_id=B.id')->where('A.is_show',1)->where('A.is_del',0)
            ->where('A.is_host',1)->where('A.stock','>',0)
            ->field('A.id,A.image,B.store_name,B.cate_id,A.price,B.unit_name,A.sales,A.people,A.sort,A.title')

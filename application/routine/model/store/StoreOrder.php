...
class StoreOrder extends ModelBasic
{
    ...
    public static function cacheKeyCreateOrderNew($uid, $productId, $addressId, $addressInfo, $payType, $payPrice,$totalNum, $useIntegral = false, $couponId = 0, $mark = '', $combinationId = 0, $pinkId = 0, $seckill_id = 0, $bargain_id = 0)
    {
        $key = md5(time());
        if (!array_key_exists($payType, self::$payType)) {
            return self::setErrorInfo('选择支付方式有误!');
        }

        if (self::be(['unique' => $key, 'uid' => $uid])) {
            return self::setErrorInfo('请勿重复提交订单');
        }
        ...
        if ($addressId == 'empty' && $realName && $phoneNo) {
            if ($address = UserAddress::set($addressInfo)) {
                UserAddress::setDefaultAddress($address->id, $uid);
            } else {
                return JsonService::fail('添加收货信息失败!');
            }
        } else {
            if (UserAddress::edit($addressInfo, $addressId, 'id')) {
                UserAddress::setDefaultAddress($addressId, $uid);
            } else {
        ...
        if ($couponId) {
            ...
            if (!$couponInfo) {
                return self::setErrorInfo('选择的优惠劵无效!');
            }
        ...
        if (!$res1) {
            return self::setErrorInfo('使用优惠劵失败!');
        }
        ...
        if (!$res2) {
            return self::setErrorInfo('使用积分抵扣失败!');
        }
        ...
        if (!$order) {
            return self::setErrorInfo('订单生成失败!');
        }

        $res5 = true;
        //减库存加销量
        if ($combinationId) {
            $res5 = $res5 && StoreCombination::decCombinationStock($totalNum, $combinationId);
        } else {
            $res5 = $res5 && StoreProduct::decProductStock($totalNum, $productId, '');
        }
    ...
    public static function cacheKeyCreateOrder($uid, $key, $addressId, $payType, $useIntegral = false, $couponId = 0, $mark = '', $combinationId = 0, $pinkId = 0, $seckill_id = 0, $bargain_id = 0)
    {
        ...
        if (!$cartGroup) {
            return self::setErrorInfo('订单已过期,请刷新当前页面!');
        }
        ...
        if (!$addressId) {
            return self::setErrorInfo('请选择收货地址!');
        }
        ...
        foreach ($cartInfo as $cart) {
            //减库存加销量
            if ($combinationId) {
                $res5 = $res5 && StoreCombination::decCombinationStock($cart['cart_num'], $combinationId);
            } else {
    ...
    public static function yuePay($order_id, $uid, $formId = '')
    {
        ...
        if ($orderInfo['pay_type'] != 'yue') {
            return self::setErrorInfo('该订单不能使用余额支付!');
        }
...
//            HookService::listen('yue_pay_product',$userInfo,$orderInfo,false,PaymentBehavior::class);
    ...
    public static function orderApplyRefund($uni, $uid, $refundReasonWap = '', $refundReasonWapExplain = '', $refundReasonWapImg = array())
    {
        ...
        if (!$order) {
            return self::setErrorInfo('支付订单不存在!');
        }
        ...
        if ($order['refund_status'] == 1) {
            return self::setErrorInfo('正在申请退款中!');
        }

        if ($order['status'] == 1) {
            return self::setErrorInfo('订单当前无法退款!');
        }
        ...
        if (!$res) {
            ...
            try {
                ...
                if (!$adminIds || !($adminList = array_unique(array_filter(explode(',', trim($adminIds)))))) {
                    return false;
                }
                ...
                foreach ($adminList as $uid) {
                    ...
                    file_put_contents('tuik.txt', $formId . $uid);
                    if ($formId) {
                        RoutineFormId::delFormIdOne($formId);
                        RoutineTemplate::sendTemplate(WechatUser::getOpenId($uid), RoutineTemplate::ORDER_REFUND_STATUS, '', $data, $formId);
                    }
                }
//                HookService::afterListen('store_product_order_apply_refund',$order['id'],$uid,false,StoreProductBehavior::class);
    ...
    public static function paySuccess($orderId, $formId = '')
    {
        ...
        if ($order->combination_id && $res1 && !$order->refund_status) {
            $resPink = StorePink::createPink($order);
        }
//创建拼团
        ...
        if ($order['pay_type'] == 'yue') {
            $data['keyword5']['value'] = '余额支付';
        } else if ($order['pay_type'] == 'weixin') {
            $data['keyword5']['value'] = '微信支付';
        }
        ...
        foreach ($adminList as $uid) {
            ...
            RoutineFormId::delFormIdOne($formId);
            RoutineTemplate::sendTemplate($openid, RoutineTemplate::ORDER_PAY_SUCCESS, '', $data, $formId);
        }
        $res = $res1 && $resPink;
        return false !== $res;
    }
    ...
    public static function takeOrder($uni, $uid)
    {
        ...
        if ($order['_status']['_type'] != 2) {
            return self::setErrorInfo('订单状态错误!');
        }

                    ...
                    <div class="m-b m-l">
                        <form action="" class="form-inline search-form">
                            ...
                            <div class="search-item" data-name="combination_id">
                                ...
                                <button type="button" class="btn btn-outline btn-link" data-value="砍价订单">砍价订单</button>
                ...
                <div class="table-responsive ibox-content" style="padding-right: 0;padding-left: 0;overflow:visible">
                    <div class="row">
                        ...
                        <div class="col-sm-12 m-b-sm">
                            <span>售出商品：<strong class="h3 text-warning m-sm">{$price.total_num}</strong></span>
                            <span>订单数量：<strong class="h3 text-warning m-sm">{$list_num.total}</strong></span>
                            <span>订单金额：<strong class="h3 text-warning m-sm">￥{$price.pay_price}</strong></span>
                            <span>退款金额：<strong class="h3 text-warning m-sm">￥{$price.refund_price}</strong></span>
                        </div>
                        <div class="col-sm-8 text-left m-b-sm">
                            {if condition="$price['pay_price_wx'] GT 0"}
                                <span>微信支付金额：<strong class="text-danger m-sm">￥{$price.pay_price_wx}</strong></span>
                            {/if}
                            {if condition="$price['pay_price_yue'] GT 0"}
                                <span>余额支付金额：<strong class="text-danger m-sm">￥{$price.pay_price_yue}</strong></span>
                            {/if}
                            {if condition="$price['pay_price_offline'] GT 0"}
                                <span>线下支付金额：<strong class="text-danger m-sm">￥{$price.pay_price_offline}</strong></span>
                            {/if}
                            {if condition="$price['pay_price_other'] GT 0"}
                                <span>线下支付金额：<strong class="text-danger m-sm">￥{$price.pay_price_other}</strong></span>
                            {/if}
                            {if condition="$price['use_integral'] GT 0"}
                                <span>积分抵扣：<strong class="text-success m-sm">{$price.use_integral} (抵扣金额:￥{$price.deduction_price})</strong></span>
                            {/if}
                            {if condition="$price['back_integral'] GT 0"}
                                <span>退回积分：<strong class="text-success m-sm">{$price.back_integral}</strong></span>
                            {/if}
                        </div>
                    </div>
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <span class="input-group-btn js-group-btn" style="min-width: 106px;">
                                    ...
                                    <button class="btn btn-primary btn-xs" type="button" onclick="$eb.createModalFrame('去发货','{:Url('deliver_goods',array('id'=>$vo['id']))}')"><i class="fa fa-cart-plus"></i> 去发货</button>
                                    <div class="btn-group">
                                        <button data-toggle="dropdown" class="btn btn-warning btn-xs dropdown-toggle"
                                                aria-expanded="false">操作
                                            <span class="caret"></span>
                                        ...
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a class="save_mark" href="javascript:void(0);" onclick="$eb.createModalFrame('去送货','{:Url('delivery',array('id'=>$vo['id']))}')">
                                    ...
                                    {elseif condition="$vo['paid'] eq 1 && $vo['refund_status'] eq 1"/}<!--已支付  退款中-->
                                      <div class="btn-group">
                                          ...
                                          <ul class="dropdown-menu">
                                              ...
                                              {/if}
                                              <li>
                                                  <a href="javascript:void(0);" onclick="$eb.createModalFrame('不退款','{:Url('refund_n',array('id'=>$vo['id']))}')">

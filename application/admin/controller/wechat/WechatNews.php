...
//            exit();
    ...
    public function message($cid = 0)
    {
        ...
        $this->assign('where',$where);
        $where['merchant'] = 0;//区分是管理员添加的图文显示  0 还是 商户添加的图文显示  1
        $content=WechatNewsModel::getMessage();
        $this->assign('cid',$cid);
    ...
    public function newmessage(){
        ...
        $news['cid'] = array();
        if($id){
            $news = WechatNewsModel::where('n.id',$id)->alias('n')->field('n.*,c.content')->join('__WECHAT_NEWS_CONTENT__ c','c.nid=n.id')->find();
            if(!$news) return $this->failedNotice('数据不存在!');
            $news['cid'] = explode(',',$news['cid']);
        }
        $all = array();
    ...
    public function addmessage(Request $request)
    {
        ...
        $content = $data['content'];
        unset($data['content']);
        $oldId=WechatNewsModel::where('type',1)->value('id');
        if ($oldId){
            WechatNewsModel::beginTrans();
            $res1 = WechatNewsModel::edit($data, $oldId, 'id');
            $res2 = WechatNewsModel::setContent($oldId, $content);
            if ($res1 && $res2)
                $res = true;
            else
                $res = false;
            WechatNewsModel::checkTrans($res);
            if ($res)
                return Json::successful('修改图文成功!', $oldId);
            else
                return Json::fail('修改图文失败!', $oldId);
        }
        if ($data['id']) {
            $id = $data['id'];
            unset($data['id']);
